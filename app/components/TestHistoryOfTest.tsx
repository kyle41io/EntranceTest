...
import {getTestAttemptsByTestId } from '../api/testAttempts'
import { AccuracyIcon } from './Icon'
import Link from 'next/link'

interface TestHistoryLOfTestProps {
    testId: number;
  }
  
const TestHistoryOfTest = ({ testId }: TestHistoryLOfTestProps) => {
  //const user = JSON.parse(localStorage.getItem("user") || "{}");
  
  const testAtemptsQuery = useQuery({
    queryKey: ["testAttempts", testId],
    queryFn: () => getTestAttemptsByTestId(testId),
  })
  return (
    <div className='flex flex-col items-center justify-start absolute w-2/3 bg-gray-300 -z-20 ml-64 p-6 overflow-y-scroll max-h-full'>
      <div className="inline-flex items-center rounded-md px-2 py-1 ring-1 ring-inset bg-blue-50 text-blue-700 ring-blue-600/20 font-semibold text-xl">Lịch sử tham gia</div>
            <table className="min-w-full divide-y divide-gray-300">
              <thead>
                <tr>
                  <th scope="col" className="py-1 pl-4 text-center text-sm font-semibold text-gray-900 sm:pl-0">
                    Email
                  </th>
                  <th scope="col" className="py-1 pl-4 text-center text-sm font-semibold text-gray-900 sm:pl-0">
                    Thời gian
                  </th>
                  <th scope="col" className="px-3 py-1 text-center text-sm font-semibold text-gray-900">
                    Bài test
                  </th>
                  <th scope="col" className="px-3 py-1 text-center text-sm font-semibold text-gray-900">
                    Số câu đúng
                ...
                {testAtemptsQuery &&
                  ...
                  .map((attempt: any, index: number) => (
                      <tr key={testId}>
                       ...
                       <td className="whitespace-nowrap px-3 py-1 text-base text-gray-500">
                         <div className="text-gray-900 text-semibold">{attempt.testName}</div>
                       </td>
   
                       <td className="whitespace-nowrap px-3 py-1 text-semibold text-gray-500">
...
export default TestHistoryOfTest